...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/zirconium/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            lunch zirconium_$target-$variant
        fi
    fi
    return $?
}
...
function aospremote()
{
    ...
    git remote rm zirconium 2> /dev/null
    ...
    git remote add zirconium https://android.googlesource.com/$PFX$PROJECT
    echo "Remote 'zirconium' created"
}
...
function repopick() {
    T=$(gettop)
    $T/vendor/zirconium/build/tools/repopick.py $@
}

function losrepopick() {
    T=$(gettop)
    $T/vendor/zirconium/build/tools/losrepopick.py $@
}
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/zirconium/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
